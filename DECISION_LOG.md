...
### 2026-03-21: ARDA Structural Hierarchy and Root Node Synthesis
* **Context:** The ARDA pipeline extracts three distinct datasets (US Religious Groups, World Religions Family Trees, and Measurements). Initially, these extracts were floating without a unifying root, and intermediate categories (like "Families" or "Measurement Categories") were being incorrectly mapped as text strings in the `Parent_IDs` column rather than resolvable alphanumeric IDs.
* **Decision:** The pipeline now synthesizes explicit master root nodes for each dataset (`US_RG`, `WORLD_TREES`, `M_ROOT`). Furthermore, it extracts hidden native IDs from the ARDA DOM (e.g., Family IDs from hyperlinks and Category IDs from hidden `<select>` dropdowns) to dynamically generate intermediate concept rows.
* **Reasoning:** SSSOM mapping requires strict ID-to-ID hierarchical relationships. Synthesizing the root nodes successfully segregates the three ontologies to prevent cross-graph pollution, while extracting native IDs for intermediate tiers correctly anchors the vertical taxonomy without hallucinating fake data.

### 2026-03-21: Transition to tripartite CDC architecture for categorization
* **Context:** The previous categorization processing pipeline relied on manual Excel Power Query imports and risked overwriting historical human audits if synthesized source IDs mutated or if the extraction pipeline was run multiple times.
* **Decision:** Replaced the insert-only categorization script with a hash-based Change Data Capture (CDC) engine. State is now managed across three isolated files: Master (Metadata), CSV (AI Log), and Excel (Human Audit).
* **Reasoning:** Generating a deterministic SHA-256 hash of the semantic columns allows the pipeline to accurately detect upstream metadata mutations. Isolating the human edits to a single `category_human` column and merging it back onto uncorrupted master data prevents human typos from corrupting the core metadata. Furthermore, syncing the Excel file programmatically using `xlsxwriter` eliminates the manual Power Query steps and protects data types from Excel's auto-formatting corruption.

### 2026-03-24: Mitigating LCSH Polyhierarchical Drift via Breadth-First Search (BFS)
* **Context:** The Library of Congress Subject Headings (LCSH) is a highly associative, polyhierarchical graph. Standard Depth-First Search (DFS) path resolution frequently resulted in severe semantic drift. For example, traversing upward from the religious concept "Confession" would often route through the legal concept "Confidential communications," resulting in an invalid, multi-domain `Hierarchy_Path`.
* **Decision:** Replaced DFS with a Breadth-First Search (BFS) algorithm for all associative graph ancestry resolution. The script explores all parents level-by-level, returning the first path that intersects with a predefined set of `CORE_ROOT_LABELS`.
* **Reasoning:** BFS mathematically guarantees the shortest, most direct path back to a conceptual root. In an associative graph like LCSH, "path length equals drift." Prioritizing the shortest path successfully filters out circuitous routes through unrelated disciplines (like law or psychology), keeping the visual breadcrumb trail conceptually pure.

### 2026-03-24: Structural Pruning of Pre-Coordinated Headings (LCSH)
* **Context:** LCSH relies heavily on pre-coordinated "Complex Subjects" (compound strings separated by double hyphens, e.g., "Caste--India" or "Theology, Practical--Methodology") to create highly specific bibliographic indices. These nodes pollute the conceptual ontology and cause valid branches to be improperly nested under geographic or methodological parents.
* **Decision:** Implemented a strict structural filter (Gate 1). The crawler explicitly rejects any node identified by the LOC API as a `http://www.loc.gov/mads/rdf/v1#ComplexSubject`, or any node whose label contains the `--` delimiter. Crucially, the BFS path builder also rejects any upward lineage that contains a complex ancestor.
* **Reasoning:** This project is scoped to mapping conceptual architecture, not bibliographic or geographical instances. Pruning complex subjects forces the crawler to stay within the "pure" topical hierarchy, preventing geographic data from corrupting the sociological or theological taxonomy.

### 2026-03-25: Consolidation to a Unified LCSH Extraction Script
* **Context:** The initial LCSH ingestion strategy used two separate scripts: one for specific taxonomic groups (using DFS) and one for broad concepts (using BFS). A review of the groups output revealed path instability (e.g., blank node IDs like `_:b121`), circuitous semantic drift (e.g., routing to the Virgin Mary via "Amusements > Amateur plays"), and structural pollution (e.g., "Japan--Civilization--Pure Land influences").
* **Decision:** Abandoned the dual-script architecture. The taxonomic seeds (e.g., "Sects", "Cults") were merged into the broad script to create a single, unified extraction pipeline powered entirely by Breadth-First Search (BFS) and strict structural gating.
* **Reasoning:** Forcing all seeds through the BFS logic guarantees that even highly specific sects receive the shortest, most direct path back to a core religious root, completely eliminating the multi-domain scenic routes and blank-node resolution errors caused by the legacy DFS crawler. 

### 2026-03-25: Acceptance of the LCSH "Purity Trap" Trade-off
* **Context:** The strict structural filter (Gate 1) drops any LCSH node tagged as a `ComplexSubject` (e.g., pre-coordinated strings like "Prayer--Catholic Church") to prevent bibliographic and geographic noise from polluting the conceptual ontology. However, we discovered this creates a "purity trap": valid downstream concepts (like "Rosary") whose only upward paths route through a Complex Subject are completely blocked from ingestion.
* **Decision:** We will maintain the strict structural block and accept the resulting data loss, rather than engineering a "permeable" BFS that crawls through complex nodes and attempts to clean the resulting path strings.
* **Reasoning:** A core goal of this project is to build a clean, structurally sound conceptual framework. Attempting to dynamically parse and strip complex pre-coordinated strings introduces significant risk of accidentally ingesting the massive volume of geographic and methodological noise that LCSH is known for. We are explicitly prioritizing strict conceptual boundaries over exhaustive vocabulary coverage.