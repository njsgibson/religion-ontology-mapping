...
* **for associative graphs:** Hardcode the seeds and crawl descendants, but impose strict depth limits (e.g., 7 levels) to prevent semantic drift into irrelevant sub-topics. Because these graphs are highly interconnected, consider applying explicit **Blocklists** to instantly halt crawling on known out-of-scope branches (e.g., in LCSH all of "Psychology" is a child of "Soul") and defining **Terminal Nodes** to act as intentional "leaves" (ingesting the boundary concept but refusing to crawl its descendants). Apply different depth limits or terminal rules to different seeds if required.
...
* **caching and resilience:** For iterative API crawls, implement a persistent cache so that if a long-running extraction fails mid-process, the script can resume from the last saved concept. To survive transient server dropouts (e.g., 502 Bad Gateway errors), implement a **Two-Phase Execution** architecture: capture timeouts in an in-memory queue during the primary crawl and execute a targeted cleanup pass on that queue at the end of the session. Explicitly log any URI that fails this secondary pass to a hard-failure CSV for manual intervention.
* **ancestry construction:** Scripts must dynamically construct the `Hierarchy_Path` by traversing *upward* from the node to the absolute root of the ontology, ensuring that the breadcrumb trail is complete regardless of the seed from which the crawl originated. For polyhierarchical associative graphs, scripts must use a **breadth-first search (BFS)** path-resolution algorithm. Because lateral associations can create circuitous, multi-domain loops, BFS mathematically guarantees the shortest, most direct path back to a core conceptual root, effectively eliminating "scenic route" semantic drift.
...
* **Topological Pruning (Human-in-the-Loop):** For massive, highly detailed graphs (e.g., Wikidata), automated depth limits and blocklists may still capture out-of-scope, hyper-specific instances deep in the tree (e.g., individual clergy members or specific local parishes). To handle this without introducing brittle, hardcoded text filters into the extraction scripts, consider incorporating a deterministic topological pruning engine. In this approach, used for Wikidata, a human reviewer audits the initially extract dataset and assigns specific action flags to target nodes. A subsequent script uses the exact `Hierarchy_Path` of these flagged concepts as a strict string prefix to systematically amputate or protect entire descendant branches. This ensures the final Bronze Layer dataset is more domain-scoped before it is passed to the global consolidation phase.
...
The DDC is a proprietary taxonomic system managed by OCLC. This project targets religion-related concepts available in the "DDC Summaries" (the top 1000 terms). Data were retrieved via the OCLC WorldCat Entities Linked Data service, which represents DDC concepts as a graph of JSON-LD objects.

* **Targeted API Crawl (Strategy B):** The pipeline executes a recursive downward crawl starting from the "Religion" root (Notation `2`) and other seeds surfaced through manual discovery (because the JSON-LD does not contain `seeAlso` or `skos:related` elements). It uses the `/narrower` and `/broader` properties of the WorldCat Entities REST API to discover and ingest descendants.
* **Identifier Management and CURIE Resolvability:** To ensure CURIEs remain globally resolvable to the OCLC service, the pipeline uses the native OCLC alphanumeric hash as the `Concept_ID` and primary key. The decimal notation (e.g., `220.1`) is preserved as metadata within the `Synonyms` column to support standard library-science search patterns.
* **Shortest-Path Ancestry Resolution:** While DDC notation implies a strict tree, the Linked Data implementation allows for associative relationships. The script uses a **Breadth-First Search (BFS)** algorithm to resolve the `Hierarchy_Path`, mathematically guaranteeing the most direct breadcrumb trail back to the 200-series root.
* **Semantic Filtering:** During ingestion, the script identifies nodes labeled "[Unassigned]." To maintain a clean "Bronze Layer" and prevent semantic noise in the search index, these nodes are excluded from the final CSV output. Their children are still evaluated to ensure no valid deeper branches are lost.
* **Licensing and Attribution:** Data is ingested under the CC BY-NC-ND 3.0 license governing DDC Summaries. Per the "NoDerivatives" clause, the pipeline performs only functional transformations (format shifting to CSV) and filtering (subsetting the 200s), preserving the original conceptual labels and hierarchical intent of the source.
...
LCSH is an associative, polyhierarchical ontology. While it uses SKOS hierarchical relationships (`skos:broader` and `skos:narrower`), it is not built as a strict taxonomy. Connections between concepts frequently jump across unrelated domains (e.g., linking a theological concept like "Confession" to a legal concept like "Confidential communications"). This creates a graph where automated downward semantic drift is a severe risk. To safely extract relevant religious groups and concepts, the pipeline uses a single extraction script incorporating strict graph-traversal guardrails:

1. **Unified targeted extraction:** The script simultaneously targets strict taxonomic seeds (e.g., "Sects", "Cults") and broad, highly associative seeds (e.g., "Religions"). To navigate this highly interconnected web efficiently, the script employs **translucent crawling**: if the downward crawl encounters a concept that has already been ingested (due to overlapping hierarchies), it skips metadata extraction but "passes through" to reach deeper descendants, safely crawling down to the designated depth limit without generating duplicate records.
2. **Structural and semantic guardrails:** To prevent the crawl from drifting into irrelevant domains (e.g., law, sociology, or general history), the pipeline strictly enforces three traversal gates:
    * **structural filtering:** Explicitly rejects any node identified by the API as a `ComplexSubject` (pre-coordinated compound strings like "Caste--India" or "Prayer--Catholic Church") to maintain conceptual purity. *Trade-off: While this successfully prevents geographic or bibliographic noise from polluting the taxonomy, it creates a "purity trap." Testing revealed a few otherwise valid concepts can only be reached via complex parents (e.g., "Rosary," which only has complex broader terms [e.g., "Beads--Religious aspects--Catholic Church"; "Mary, Blessed Virgin, Saint--Devotion to"; "Prayer--Catholic Church"]). Such concepts are not currently ingested.*
    * **drift blocklists:** Instantly halts crawling on explicitly defined out-of-scope URIs (e.g., "Psychology") before they can be ingested or expanded.
    * **terminal nodes:** Ingests predefined boundary concepts (e.g., "Marriage") but acts as a "leaf," refusing to crawl their narrower children.
3. **Shortest-path ancestry resolution:** Because LCSH concepts frequently possess multiple broader parents, following a random upward path often leads to multi-domain, circuitous breadcrumbs. To build the `Hierarchy_Path`, the script reverse-engineers the upper hierarchy using a **breadth-first search (BFS)** algorithm. It explores all parents level-by-level, mathematically guaranteeing the shortest, most direct path back to a core religious root, thereby filtering out complex or non-religious lineages.
4. **Crosswalk extraction:** The Library of Congress links its Subject Headings to external authorities (e.g., matching a subject heading to a Getty AAT concept). The script extracts these links by querying `skos:exactMatch`, `skos:closeMatch`, and `mads:hasCloseExternalAuthority` properties, passing them directly to the `Crosswalks` column for future harmonization.
...
The UDC Summary is a multilingual classification scheme managed by the UDC Consortium. It is natively published as Linked Open Data (LOD) using the W3C SKOS standard. Because the official `udcdata.info` endpoint is (as of March 2026) undergoing offline revision, and the fallback web interface (`udcsummary.info`) relies on dynamic JavaScript rendering that makes traditional web scraping brittle, the pipeline relies on local memory parsing of the RDF graph.

* **Local Bulk Parsing (Strategy A):** The pipeline uses `rdflib` to load a downloaded, static copy of the CC BY-SA 3.0 `udcs-skos.rdf` file directly into local memory. This bypasses the need for network calls entirely, allowing instantaneous graph traversal.
* **Identifier Alignment and Decimal Notation:** To comply with SSSOM standards for Linked Open Data, the pipeline extracts the native integer from the URI (e.g., `015988`) to serve as the `Concept_ID` and the base for the `CURIE` (e.g., `UDC:015988`). The traditional decimal classmark (e.g., `2-1` for "Theory and philosophy of religion") extracted from `skos:notation` is explicitly routed to the `Synonyms` column to ensure it remains indexed for search applications without breaking semantic web resolvability. 
* **Multilingual Array Filtering:** The UDC Summary is translated into over 50 languages. The script strictly filters `skos:prefLabel`, `skos:altLabel`, `skos:hiddenLabel`, `skos:scopeNote`, and `skos:note` to extract only English (`@en`) strings. The presence of non-English strings dynamically triggers the `Has_Translation` flag. 
* **Shortest-Path Ancestry Resolution:** The script uses a **Breadth-First Search (BFS)** algorithm to traverse `skos:broader` links and mathematically construct the absolute `Hierarchy_Path` back to the Class 2 (Religion) root. 
* **Lateral Discovery:** UDC extensively uses `skos:related` to cross-link distinct disciplines (e.g., linking "Religion" to "Philosophy"). These lateral links are intentionally excluded from the primary Bronze Layer extraction to maintain the strict vertical taxonomy. They are instead queried in a secondary script to populate the ephemeral lateral candidate queue for human review.

### Wikidata

Wikidata is a massive, crowdsourced, polyhierarchical knowledge graph published as Linked Open Data. It does not possess a single structural root for religious concepts; rather, domain concepts are scattered across disparate upper-ontology hubs (e.g., "belief", "practice", "organization", "building"). Furthermore, its crowd-sourced nature results in extreme polyhierarchy and the deep nesting of hyper-specific instances (e.g., individual bishops) and geographic intersections (e.g., "Buddhism in Japan"). Ingesting Wikidata requires a highly defensive, multi-stage API crawling strategy to protect the semantic purity of the final taxonomy:

1. **Multi-seed downward crawl (Strategy B):** Because downloading the full Wikidata graph is computationally prohibitive, the pipeline executes a recursive downward crawl using the live REST API (`wbgetentities`). It starts from a curated array of explicit upper-ontology seeds (e.g., `Q9174` Religion, `Q2110808` Religious behavior) and crawls to a strict depth limit (e.g., 7) to prevent unbounded semantic drift.
2. **Translucent crawling for polyhierarchy:** Because concepts in Wikidata frequently have multiple parents, the crawler will often encounter the same node via different descending branches. The script uses an in-memory dictionary to cache ingested nodes. If a node is encountered a second time, the script performs a "translucent" pass-through—it skips the redundant API metadata extraction but continues crawling downward to ensure no deep descendants are orphaned.
3. **Dynamic geographic pruning:** Wikidata heavily categorizes concepts by geographic or administrative slices (e.g., "Religion on the Earth", "Hinduism in Switzerland"). To protect the conceptual ontology from geographic noise, the crawler applies a real-time lexical filter. It drops any concept ending in specific hub suffixes (e.g., "on the Earth", "by country") and actively scans the concept's intrinsic parent labels to drop contextual slices (e.g., dropping a node if its label starts with `[Parent_Label] in `).
4. **Shortest-path ancestry resolution (BFS):** Because a node can have dozens of overlapping lineages, following a random upward path often yields a circuitous or irrelevant breadcrumb trail. The script uses a **Breadth-First Search (BFS)** algorithm to reverse-engineer the upper hierarchy. It mathematically guarantees the shortest, most direct path back to one of the explicit core seeds. If the BFS engine cannot find a path to a root without passing through a geographic hub, the node is flagged as a detached geographic isolate and proactively dropped.
5. **Heuristic path deduplication:** Running multiple seed crawls invariably produces duplicate extractions of the same concept with different `Hierarchy_Path` routes (e.g., routing "Quranism" through "Textualism" versus "Islam"). The pipeline uses a post-extraction Smart Merge script. It deduplicates rows based on their `CURIE` and applies a heuristic scoring algorithm to the overlapping paths—rewarding domain-native taxonomy branches (e.g., paths containing "belief system" or "theology") and heavily penalizing geographic or broken paths. The highest-scoring path is retained.
6. **Human-in-the-Loop topological pruning:** To surgically remove deep branches of extreme instances (e.g., thousands of specific bishops) without hardcoding text rules into the crawler, the pipeline uses a prefix-based topology engine. A human reviewer audits the initial raw CSV output and assigns strict action codes to specific nodes. The engine recognizes four hierarchical commands:
   * **`floor`**: Keeps the flagged node, but drops all of its descendants.
   * **`prune`**: Drops the flagged node and all of its descendants.
